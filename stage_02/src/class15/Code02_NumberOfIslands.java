...
public class Code02_NumberOfIslands {
    ...
    public static int numIslands1(char[][] board) {
        ...
        Dot[][] dots = new Dot[row][col]; // 用dot代表'1'
        List<Dot> dotList = new ArrayList<>(); // dot数组
        for (int i = 0; i < row; i++) {
            for (int j = 0; j < col; j++) {
                if (board[i][j] == '1') {
                    dots[i][j] = new Dot();
        ...
        for (int j = 1; j < col; j++) {
            // (0,0)跳过了
            if (board[0][j - 1] == '1' && board[0][j] == '1') { // 左边是'1'，自己也是'1'
                uf.union(dots[0][j - 1], dots[0][j]); // 合并dot
            }
        }

        // 第0列
        for (int i = 1; i < row; i++) {
            if (board[i - 1][0] == '1' && board[i][0] == '1') { // 上边是'1'，自己也是'1'
                uf.union(dots[i - 1][0], dots[i][0]); // 合并dot
            }
        }
        ...
        for (int i = 1; i < row; i++) {
            for (int j = 1; j < col; j++) {
                if (board[i][j] == '1') { // 自己是'1'
                    if (board[i][j - 1] == '1') { // 左边是'1'
                        uf.union(dots[i][j - 1], dots[i][j]); // 合并dot
                    }
                    if (board[i - 1][j] == '1') { // 上边是'1'
                        uf.union(dots[i - 1][j], dots[i][j]); // 合并dot
                    }
                }
            }
        }
